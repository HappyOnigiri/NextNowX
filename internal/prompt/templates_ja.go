...
// `nnx` のコマンド例・ステータス値は原語のまま残す。
// docs/design/agent-prompts.md を参照。

const japaneseDesignTemplate = `feature {{feature_id}} の Next Now X task {{task_id}} を設計する。

Title: {{task_title}}
Scope: {{task_scope}}

Next Now X は task と task 同士の依存関係を追跡するローカルの CLI である。
正確なコマンド体系は ` + "`nnx --help`" + ` と ` + "`nnx <command> --help`" + ` で確認する。
Next Now X はこのマシンの中だけで動くので、リポジトリを読む人からは見えない。
リポジトリが運ぶものに持ち込まないこと。コードコメント・コミットメッセージ・pull request の
いずれも Next Now X やその識別子・コマンドに言及してはならない。
...
2. task と、それが依存している作業を読む。
   ...
   graph にまだない依存は ` + "`nnx dependency add BLOCKER_TASK_ID {{task_id}}`" + ` で登録する。
3. 何かを決める前に、Next Now X に添付された資料を読む。
   document は task・その feature・その feature が属する project のそれぞれに付き、
   どれにもこの scope が満たすべき要件が入っている可能性がある。
   - ` + "`nnx document --task {{task_id}}`" + ` と ` + "`nnx document --feature {{feature_id}}`" + `
   - ` + "`nnx feature {{feature_id}}`" + ` が project を示すので、続けて ` + "`nnx document --project PROJECT_ID`" + `
   - ` + "`nnx document get DOCUMENT_ID`" + ` は保存済みの document を出力する。URL やローカル
...
const japaneseImplementationTemplate = `feature {{feature_id}} の Next Now X task {{task_id}} を実装する。
...
1. task と、それが依存している作業と、登録済みの計画を読む。
   ...
   計画は登録しない。
2. コードを書く前に、Next Now X に添付された資料を読む。
   document は task・その feature・その feature が属する project のそれぞれに付き、
   どれにもこの scope が満たすべき要件が入っている可能性がある。
   - ` + "`nnx document --task {{task_id}}`" + ` と ` + "`nnx document --feature {{feature_id}}`" + `
   - ` + "`nnx feature {{feature_id}}`" + ` が project を示すので、続けて ` + "`nnx document --project PROJECT_ID`" + `
...
5. 結果を Next Now X に記録する。
   - pull request として着地する作業: 分岐したベースに対して pull request を開き、
     ` + "`nnx pr attach {{task_id}} PULL_REQUEST_URL`" + ` を実行する。
     その後の状態は pull request に従うので、ステータスを手で設定しない。
   - pull request を伴わない作業: 終わったら ` + "`nnx task update {{task_id}} --status completed`" + ` を実行する。

変更した内容と、計画が扱っていなかった点を報告する。
`

const japaneseBatchTemplate = `feature {{feature_id}} の、以下に挙げる Next Now X task を実装する。

Next Now X は task と task 同士の依存関係を追跡するローカルの CLI である。
正確なコマンド体系は ` + "`nnx --help`" + ` と ` + "`nnx <command> --help`" + ` で確認する。
Next Now X はこのマシンの中だけで動くので、リポジトリを読む人からは見えない。
自分の作業でも各 SubAgent の作業でも、リポジトリが運ぶものに持ち込まないこと。コードコメント・
コミットメッセージ・pull request のいずれも Next Now X やその識別子・コマンドに言及してはならない。
...
2. 各 task をそれぞれ専用の SubAgent に渡す。1 つの SubAgent に 1 つの task だけを渡し、
   ...
   各 SubAgent は指示を自分からではなく Next Now X から受け取る。
   - 渡された task について ` + "`nnx prompt TASK_ID --kind implementation`" + ` を実行し、
...
const japaneseBatchDesignTemplate = `feature {{feature_id}} の、以下に挙げる Next Now X task を設計する。
...
2. 各 task をそれぞれ専用の SubAgent に渡す。1 つの SubAgent に 1 つの task だけを渡し、
   ...
   互いの書きかけの編集をコミットしてしまう。
   各 SubAgent は指示を自分からではなく Next Now X から受け取る。
   - 渡された task について ` + "`nnx prompt TASK_ID --kind design`" + ` を実行し、表示された
     プロンプトに従う。
   - そのプロンプトは設計の形を左右する判断をユーザーに質問するよう指示するが、答えられる人は
     いないので、質問せず仮定として明記して計画に書く。
   - 最後に ` + "`nnx plan set TASK_ID --file PATH`" + ` で計画を登録して終える。
