...
- **语言：** `prx setup` 会询问使用英语还是日语，并保存该选择。之后可用 `prx config language update auto|en|ja` 修改。
- **示例数据：** 首次执行 `prx setup` 时，如果它创建了数据库，就会用你所选的语言添加一个小型示例 project。不需要时请使用 `prx setup --no-sample-data`。
