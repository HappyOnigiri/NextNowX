...
func newOpenService(_ io.Writer) cli.OpenService {
	return func(ctx context.Context, options cli.ServiceOptions) (cli.Service, io.Closer, error) {
		...
		if options.Demo {
			var err error
			temporaryRoot, err = os.MkdirTemp("", "nnx-demo-")
			if err != nil {
				return nil, nil, err
			}
			dbPath = filepath.Join(temporaryRoot, "nnx.db")
		...
		service.SetDaemonInspector(func(context.Context) domain.DebugDaemonInput {
			return daemon.Inspect(launchd.New(), nnx.Version()).DebugInput(options.Demo)
		})
		setUpdateSources(service)
