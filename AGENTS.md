...
- 例外は組み込みプロンプトテンプレートの本文と、`prx setup` が投入するサンプルデータの本文で、これらだけは実効言語で英語版と日本語版を切り替える。両方の版を維持する。実効言語の決め方は `docs/design/agent-prompts.md` にある。CLI のヘルプ・標準出力・エラーメッセージは、サンプル投入を知らせる 1 行も含めて英語のままにする。
