export const shell = {
  en: {
    app: { title: "Next Now X" },
    settings: {
      open: "Settings",
      title: "Settings",
      tabs: {
        server: "Server",
        prompts: "Prompts",
        labels: "Labels",
        display: "Display",
        debug: "Debug",
        licenses: "Licenses",
      },
      display: {
        language: {
          label: "Display language",
          options: { auto: "Automatic", en: "English", ja: "日本語" },
          note: "Also selects the language of the built-in agent prompts.",
          error: "The language could not be saved.",
        },
        theme: {
          label: "Display theme",
          options: { system: "System", light: "Light", dark: "Dark" },
        },
      },
      debug: {
        generatedAt: "Taken at {{time}}",
        reload: "Take a new report",
        copy: "Copy report",
        loading: "Collecting the diagnostic report…",
        error: "The diagnostic report is unavailable.",
        problems: "Detected problems",
        noProblems: "No problems were detected.",
        evidence: "Evidence",
        next: "Next command",
        report: "Report",
        copyNote:
          "The copied text is English and includes this browser's own state.",
      },
      licenses: {
        loading: "Loading license information…",
        error: "License information is unavailable.",
        empty: "No license information is available.",
      },
    },
    nav: {
      dashboard: "Next Now X dashboard",
      primary: "Next Now X navigation",
      projects: "Projects",
      newProject: "New project",
      overview: "Overview",
      toggleProject: "Expand or collapse {{title}}",
      taskSearch: "Task search",
      serverUnavailable: "Server unavailable",
      liveUpdatesStopped: "Automatic updates stopped",
      hideRail: "Hide the sidebar",
      showRail: "Show the sidebar",
      resizeRail: "Sidebar width",
    },
    demo: {
      dismiss: "Hide the demo notice until the demo server restarts",
    },
    serverSettings: {
      loading: "Loading server settings…",
      syncTitle: "Automatic GitHub status refresh",
      syncInterval: "Interval in seconds",
      syncHint:
        "Minimum 600 seconds. The refresh runs while the CLI or WebUI is active.",
      syncInvalid: "The interval must be a whole number of 600 or more.",
      hostsTitle: "GitHub hosts",
      addHost: "Register a host",
      newHost: "New host",
      hostFields: "Settings for {{host}}",
      hostsInvalid: "Every host needs a name, and the names must not repeat.",
      host: "Host",
      webUrl: "Web URL",
      apiUrl: "API URL",
      uploadUrl: "Upload URL",
      graphqlUrl: "GraphQL URL",
      authTitle: "Authentication methods",
      addAuth: "Register a credential",
      newAuth: "New credential",
      authFields: "Settings for {{id}}",
      authInvalid:
        "Every credential needs a unique ID, a registered host, and the fields its source asks for.",
      authId: "Method ID",
      authType: "Credential source",
      account: "Keychain account",
      service: "Keychain service",
      variable: "Environment variable",
      token: "Inline token",
      tokenKeep: "Leave blank to keep the current token",
      user: "gh CLI user",
      userOptional: "Default account",
      keychain: "Keychain",
      environment: "Environment variable",
      inline: "Inline token",
      ghCli: "gh CLI",
      noAuth: "No explicit methods; the CLI will use GitHub.com defaults.",
      moveUp: "Move {{id}} up",
      moveDown: "Move {{id}} down",
      removeHostAction: "Remove {{host}}",
      removeAuthAction: "Remove {{id}}",
    },
  },
  ja: {
    app: { title: "Next Now X" },
    settings: {
      open: "設定",
      title: "設定",
      tabs: {
        server: "サーバー",
        prompts: "プロンプト",
        labels: "ラベル",
        display: "表示",
        debug: "デバッグ",
        licenses: "ライセンス",
      },
      display: {
        language: {
          label: "表示言語",
          options: { auto: "自動", en: "English", ja: "日本語" },
          note: "組み込みのエージェント用プロンプトの言語も切り替えます。",
          error: "言語を保存できませんでした。",
        },
        theme: {
          label: "表示テーマ",
          options: {
            system: "システム",
            light: "ライト",
            dark: "ダーク",
          },
        },
      },
      debug: {
        generatedAt: "取得時刻 {{time}}",
        reload: "取得しなおす",
        copy: "レポートをコピー",
        loading: "診断レポートを取得しています…",
        error: "診断レポートを取得できません。",
        problems: "検出された問題",
        noProblems: "問題は検出されませんでした。",
        evidence: "根拠",
        next: "次に実行するコマンド",
        report: "レポート",
        copyNote:
          "コピーされるテキストは英語で、このブラウザーの情報も含みます。",
      },
      licenses: {
        loading: "ライセンス情報を読み込んでいます…",
        error: "ライセンス情報を読み込めません。",
        empty: "表示できるライセンス情報がありません。",
      },
    },
    nav: {
      dashboard: "Next Now X ダッシュボード",
      primary: "Next Now X ナビゲーション",
      projects: "プロジェクト",
      newProject: "プロジェクトを作成",
      overview: "概要",
      toggleProject: "{{title}}を展開または折りたたむ",
      taskSearch: "タスク検索",
      serverUnavailable: "サーバーに接続できません",
      liveUpdatesStopped: "自動更新が停止中",
      hideRail: "サイドバーを隠す",
      showRail: "サイドバーを表示",
      resizeRail: "サイドバーの幅",
    },
    demo: {
      dismiss: "デモの警告をデモサーバーの再起動まで隠す",
    },
    serverSettings: {
      loading: "サーバー設定を読み込んでいます…",
      syncTitle: "GitHub ステータスの自動更新",
      syncInterval: "間隔（秒）",
      syncHint: "最小600秒。CLIまたはWebUIの利用中に更新します。",
      syncInvalid: "間隔は600以上の整数で指定してください。",
      hostsTitle: "GitHubホスト",
      addHost: "ホストを登録",
      newHost: "新しいホスト",
      hostFields: "{{host}}の設定",
      hostsInvalid: "ホスト名は必須で、重複できません。",
      host: "ホスト",
      webUrl: "Web URL",
      apiUrl: "API URL",
      uploadUrl: "Upload URL",
      graphqlUrl: "GraphQL URL",
      authTitle: "認証方法",
      addAuth: "認証を登録",
      newAuth: "新しい認証",
      authFields: "{{id}}の設定",
      authInvalid:
        "認証IDは重複しない値が必須です。ホストは登録済みのものを選び、認証元が求める項目を埋めてください。",
      authId: "認証ID",
      authType: "認証元",
      account: "Keychainアカウント",
      service: "Keychainサービス",
      variable: "環境変数",
      token: "インラインToken",
      tokenKeep: "現在のTokenを維持する場合は空欄",
      user: "gh CLIユーザー",
      userOptional: "デフォルトアカウント",
      keychain: "Keychain",
      environment: "環境変数",
      inline: "インラインToken",
      ghCli: "gh CLI",
      noAuth: "明示的な認証はありません。GitHub.comの既定候補を使います。",
      moveUp: "{{id}}を上へ移動",
      moveDown: "{{id}}を下へ移動",
      removeHostAction: "{{host}}を削除",
      removeAuthAction: "{{id}}を削除",
    },
  },
} as const;
