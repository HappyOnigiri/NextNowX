...
`nnx debug` とその RPC は公開契約であり、読み手は 1 種類しかいない。Next Now X が動かない理由の説明を求められた人、あるいはエージェントである。
...
そのため `nnx debug` はデータベースを開けなくても成功し、その失敗をストレージのセクションとして報告する。
...
`daemon_plist_stale`・`daemon_not_running`・`daemon_binary_outdated` の 3 つの問題は、LaunchAgent が導入済みのときにだけ報告する。未導入は問題ではない。`nnx serve` を手で使う運用も正当である。
...
すべての行が必要な呼び出し側は代わりに `nnx snapshot --json` を使う。
