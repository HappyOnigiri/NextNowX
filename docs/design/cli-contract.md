...
出力形式は `--json` の有無だけで決まる。テキスト出力は端末幅や周囲の環境によって変わらない。

成功した JSON コマンドは、`schema_version`・`ok`・`data` といったエンベロープを介さず、データオブジェクトを直接出力する。
空のコレクションは `[]` であり、`null` にはしない。
失敗した JSON コマンドはバージョン付きのエラーオブジェクトを出力する。現在のバージョンは `nnx schema-version` が所有する。
...
コマンドごとの読み取り形式は [生成された CLI リファレンス](../cli/nnx.md) が所有する。

`nnx config` の下の共有設定は、値を読む裸のサブコマンドと、それを書き換える `update` サブコマンドの組にする。自動同期の間隔、サーバの待ち受けポート、表示とプロンプトが共有する言語がこれにあたる。設定ファイルを直接編集しなくても切り替えられるようにするためである。

task ラベルは `nnx config label`、`nnx project label`、`nnx feature label` の `set`・`unset` で
文字列と色を独立して変更する。`set`・`unset` のキーは `status.*` または `block.*` の安定値で、
未指定の項目は変更せず、空文字列は継承へ戻す。人間向けの task 表示だけが実効文字列を使い、
JSON の `status`・`display_state`・`block_labels` は安定値を保つ。
...
`nnx setup` は、起動時にデータベースファイルを新規作成したときだけサンプルデータを投入し、投入したときだけ標準出力へ実効言語で 1 行を出す。
投入を止める `--no-sample-data` は `nnx setup` のローカルフラグで、環境変数 `NNX_NO_SAMPLE_DATA` は空でない値をすべて opt-out として扱う。
投入の成否は終了ステータスに影響せず、失敗は stderr の警告 1 行になる。条件と背景は [persistence.md](persistence.md) にある。

稼働中サーバの発見はランタイムの状態ファイルが所有する。CLI がサーバのポートを推測することはない。
CLI コマンドは常駐サーバを経由せず、直接 SQLite を開く。詳細は [daemon.md](daemon.md) にある。
