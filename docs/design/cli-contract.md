...
失敗した JSON コマンドはバージョン付きのエラーオブジェクトを出力する。現在のバージョンは `prx schema-version` が所有する。
...
`prx config` の下の共有設定は、値を読む裸のサブコマンドと、それを書き換える `update` サブコマンドの組にする。自動同期の間隔、サーバの待ち受けポート、表示とプロンプトが共有する言語がこれにあたる。設定ファイルを直接編集しなくても切り替えられるようにするためである。

task ラベルは `prx config label`、`prx project label`、`prx feature label` の `set`・`unset` で
