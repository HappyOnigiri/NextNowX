...
task の状態は 2 系統に分かれる。ステータスは 1 つで作業がどこまで進んだかを示し、ブロックラベルは 0〜4 個で進行を妨げている事情を示す。
...
CI のラベルは pull request の CI が failure のときだけ付ける。実行中（pending）とチェックなし（none）では付けない。同期間隔で拾う 1 値なので、実行中を妨害として出すと同期のたびに表示が揺れるだけになる。
CI の状態そのものは failure 以外も保存し、pull request の表示にはそのまま出す。ラベルは「今が妨害か」を、保存した値は「CI が何を報告したか」を担う。
