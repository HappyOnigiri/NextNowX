...
loopback 以外に公開するには、`prx serve --addr` で listen アドレスを明示する必要がある。LaunchAgent はこのフラグを渡さないので、常駐サーバが loopback の外に出ることはない。
...
実行は非対話で、上限時間を伴う。開発ビルドと `prx serve --demo` ではこの RPC 自体が無効である。
